...
export default function ExplorePage() {
  ...
  useEffect(() => {
    async function fetchUserAndFollowing() {
      ...
      if (user) {
        ...
        if (!error && following) {
          setFollowingIds(following.map((f: any) => f.following))
        }
      }
    }
  ...
  async function handleFollow(userId: string) {
    ...
    const { error: followError } = await supabase
      .from('follows')
      .insert({ follower: currentUserId, following: userId })
              ...
              <div className="grid gap-4">
                {userResults.map(user => {
                  ...
                  return (
                    <div
                      key={user.id}
                      className="bg-black/20 backdrop-blur-sm border border-white/10 rounded-2xl p-4 hover:bg-black/30 transition-all duration-300 group"
                    >
                          ...
                          <div className="relative">
                            {user.avatar_url ? (
                              <Image
                                ...
                                width={48}
                                height={48}
                                className="rounded-full border-2 border-white/10 group-hover:border-blue-500/30 transition-colors duration-300"
                              />
                            ) : (
